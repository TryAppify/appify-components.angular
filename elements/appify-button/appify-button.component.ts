...
export class AppifyButtonComponent implements OnInit {
    ...
    ngOnChanges(changes: SimpleChanges) {
        if (changes.style && changes.style.currentValue) {
            ...
            if (current.fillType) {
                this.style.fillType = current.fillType;
            }
            if (current.normalState) {
                this.style.normalState.color = current.normalState.color;
            }
            if (current.normalState) {
                this.style.normalState.textColor =
                    current.normalState.textColor;
            }
            if (current.hoverState) {
                this.style.hoverState.color = current.hoverState.color;
            }
            if (current.hoverState) {
                this.style.hoverState.textColor = current.hoverState.textColor;
            }
        }
    }
    ...
    backgroundColor() {
        ...
        if (this.style.normalState == null) {
            return "";
        }
        if (this.style.hoverState == null) {
            return "";
        }
        ...
        if (this.isHovering) {
            ...
            if (
                this.style.fillType == this.buttonFillTypeValue.border ||
                this.style.fillType == this.buttonFillTypeValue.none
            ) {
                return "transparent";
            }
            if (this.style.normalState && this.style.normalState.color) {
                return this.style.normalState.color;
            }
        }
    ...
    borderColor() {
        ...
        if (this.style.fillType == this.buttonFillTypeValue.border) {
            return this.style.normalState.color;
        }
        return "transparent";
    }
    ...
    color() {
        ...
        if (this.isHovering) {
            return this.style.hoverState.textColor;
        }
